...
export default function ItemDetailPage() {
  ...
  // Replace this with your actual user context or auth logic
  const loggedInUserId = localStorage.getItem("user_external_id");
  useEffect(() => {
  ...
  const handleSaveStock = async () => {
    ...
    try {
      ...
      if (response.data.status?.success) {
        toast.success(response.data.status.message || "Stock updated successfully");
                  ...
                  {loadingMovements ? (
                    ...
                    movements.map((m, idx) => (
                      <tr key={idx} className="hover:bg-gray-50">
                        ...
                        <td className="px-4 py-2">{m.created_at ? new Date(m.created_at).toLocaleDateString('en-GB', { day: '2-digit', month: 'short', year: 'numeric' }) : '-'}</td>
                        ...
                        <td className="px-4 py-2">
                          {m.status ? (
                            <span className={`px-3 py-1 rounded-full text-xs font-semibold ${m.status === 'Fully Paid' ? 'bg-green-50 text-green-600' : 'bg-gray-100 text-gray-500'}`}>{m.status}</span>
                          ) : (
                            <span className="px-3 py-1 rounded-full text-xs font-semibold bg-gray-100 text-gray-500">None</span>
                          )}
                        </td>
                      </tr>
                    ))
